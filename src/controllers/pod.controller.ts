...
export const uploadProofOfDelivery: RequestHandler = async (request, response, next) => {
  try {
    ...
    const delivery = await prisma.delivery.findUnique({
      where: { id: deliveryId },
      select: { id: true, status: true, assignedDriverId: true, podImageUrl: true }
    });
    ...
    if (delivery.podImageUrl) throw new AppError(409, 'POD_ALREADY_EXISTS', 'Proof of Delivery has already been uploaded.');
